...
# %%e extra price range per category analysis for report writing

price_range = data.groupby("product_category_name_english")["price"].agg(['min', 'max'])
...
y_values = range(len(price_range.index))


plt.barh(y_values, price_range['price_range'], color='skyblue')

# plot title and font
plt.title('Prices ranges per product category', fontdict={'fontsize': 16, 'fontweight': 'bold'})
plt.xlabel('Price range', fontdict={'fontsize': 14, 'fontweight': 'bold'})
plt.ylabel('Product categories', fontdict={'fontsize': 14, 'fontweight': 'bold'})

# use de prijscategorieën as y-labels with bigger fonts
plt.yticks(y_values, price_range.index, fontsize=12)
